...
- **Channels are independent failure domains (constitution XXVI).** Where a function has parallel hardware paths — displays above all (GPU DisplayPort vs BMC/VGA) — treat each as an independent channel: dissimilar driver stack, its own config artifact, its own health check, no shared config that lets one fault disable another. A connected-but-dark output is a latent fault POST WARNs on every run. Any display/channel fix is a per-channel rule in `post/features/platform/display.pl` — the reliability invariants (independence, idempotence, re-sensing, drift-detection) have no other shape. Never encode a static assumption about what is plugged in; derive from what is sensed (the AST1150 *bridge* at 0e:00.0 vs the ASPEED *graphics* at 0f:00.0 is a live trap — filter on the VGA class).
- **Display model is PRIME, VGA-primary.** X always boots on the ASPEED BMC head (`modesetting`, the permanent robust *spine*); the RTX A4000 is a render-offload + compute device, reached with `prime-run <app>`. The desktop survives an nvidia driver death outright — X never depends on the GPU. Base desktop is 1080p, software-composited; GPU work opts in per-app. The BMC's iKVM is the out-of-band lifeline POST verifies, so a botched X config can never lock the operator out. Dells on the GPU DP are an optional nvidia-dependent output-offload enhancement, deliberately out of the boot path.
