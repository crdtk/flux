...
# ----------------------------------------------------------
# One-shot: standardize this folder's name to match the rig's clone
# (~/Desktop/Projects/flux) WITHOUT losing Claude Code history. Claude
# keys transcripts+memory by munged cwd under ~/.claude/projects/, so
# the project dir and its key dir must move together. Refuses while a
# claude process has this directory as cwd — a live session would
# recreate the old key on its next transcript write and split history.
# Run from a plain terminal, not from inside a Claude session.
# ----------------------------------------------------------

FLUX_DIR       := $(dir $(CURDIR))flux
CLAUDE_KEY_OLD := $(USER_HOME)/.claude/projects/$(subst /,-,$(CURDIR))
CLAUDE_KEY_NEW := $(USER_HOME)/.claude/projects/$(subst /,-,$(FLUX_DIR))

.PHONY: flux-rename
flux-rename:
	@test "$(notdir $(CURDIR))" != "flux" || { echo ">>> already standardized: $(CURDIR)"; exit 0; }
	@! test -e $(FLUX_DIR) || { echo ">>> $(FLUX_DIR) already exists — refusing"; exit 1; }
	@! test -e $(CLAUDE_KEY_NEW) || { echo ">>> $(CLAUDE_KEY_NEW) already exists — merge by hand"; exit 1; }
	@for p in $$(pgrep -x claude); do \
	    test "$$(readlink /proc/$$p/cwd)" != "$(CURDIR)" || \
	        { echo ">>> a Claude session is open in $(CURDIR) — quit it first"; exit 1; }; \
	done
	mv $(CURDIR) $(FLUX_DIR)
	@! test -d $(CLAUDE_KEY_OLD) || mv $(CLAUDE_KEY_OLD) $(CLAUDE_KEY_NEW)
	@echo ">>> renamed. cd $(FLUX_DIR) && claude --resume picks up all conversations"
