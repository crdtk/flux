#!/usr/bin/env bash
# Start baseline (port 8001) then TurboQuant (port 8000) vLLM servers sequentially.
...
# --- TurboQuant (port 8000) — starts AFTER baseline is healthy ---
echo ">>> starting TurboQuant :8000  log=/tmp/vllm-tq.log"
TURBOQUANT=1 "${VENV}/bin/vllm" serve "${MODEL}" \
    --port 8000 \
    --served-model-name default \
    --gpu-memory-utilization 0.42 \
    --max-model-len 8192 \
    ...
    --enforce-eager \
    > /tmp/vllm-tq.log 2>&1 &
echo $! > /tmp/vllm-tq.pid
_wait_healthy 8000 tq 240

echo ">>> both servers ready (baseline :8001, TQ :8000)"
